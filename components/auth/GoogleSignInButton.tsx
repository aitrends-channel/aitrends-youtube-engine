...
export function GoogleSignInButton({ next, onError }: Props) {
  ...
  async function handleClick() {
    ...
    // On success Supabase redirects the browser to Google. We only reach
    // here on a client-side error (e.g. network); the loading state can
    // stay set otherwise since the page is about to be replaced.
    if (error) {
  ...
  return (
    ...
    >
      {loading ? <Spinner size={14} /> : <GoogleIcon />}
